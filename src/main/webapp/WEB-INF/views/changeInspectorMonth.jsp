...
<head>
    ...
    <title>Zmiana miesiąca dla inspektora</title>
                        ...
                        <c:forEach items="${contractMap}" var="contract">
                            <tr>
                                ...
                                <td>
                                    <c:forEach var="daysAmountList" items="${contract.value.listDaysAmount}">
                                        <c:if test="${daysAmountList.monthNumber == monthValue && daysAmountList.year == year}">
                                            <a href="/listAttendance/view?insp=${inspector.id}&conDet=${contract.value.id}&con=${contract.key.id}&dayAmountId=${daysAmountList.id}&monthValue=${monthValue}&year=${year}" class="btn btn-danger rounded-0 text-light m-1">Uzupełnij listę obecności</a>
                                        </c:if>
                                    </c:forEach>
                                </td>
                            </tr>
